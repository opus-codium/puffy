...
module Melt
  RSpec.describe RuleFactory do
    ...
    it 'passes addresses and networks' do
      result = RuleFactory.build(dst: { host: '192.0.2.1' })

      expect(result.count).to eq(1)
      expect(result[0].dst[:host].to_s).to eq('192.0.2.1')

      result = RuleFactory.build(dst: { host: '192.0.2.0/24' })
    ...
    it 'resolves hostnames' do
      ...
      result = RuleFactory.build(dst: { host: 'example.com' })
    ...
    it 'does not mix IPv4 and IPv6' do
      ...
      result = RuleFactory.build(src: { host: 'example.net' }, dst: { host: 'example.com' })
